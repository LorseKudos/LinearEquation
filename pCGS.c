...
void sparse_matvec2(int *IAP,int *JA,double *A,double *p,double *q,double *Ap){
  int i,j;
...
#pragma omp for
    for(i=0;i<N;i++){
      Ap[i] = ZERO;
      for(j=IAP[i];j<IAP[i+1];j++){
        Ap[i] = Ap[i] + (A[j])*(p[JA[j]]+q[JA[j]]); //A*(p+q)の計算
      }
    }
  }
  return;
...
int main(int argc,char *argv[]){
  ...
  if(x==NULL || r==NULL || s==NULL || u==NULL || p==NULL || q==NULL || b==NULL ||Ap==NULL || Aq==NULL || IAP==NULL || JA==NULL || A==NULL){
...
#pragma omp parallel for
    for(j=0;j<N;j++){ //xの更新
      x[j] = x[j] + alpha*(u[j]+q[j]);
    }

    sparse_matvec2(IAP,JA,A,u,q,Ap); //ApにA*(u+q)を代入
#pragma omp parallel for
    for(j=0;j<N;j++){ //rの更新
      r[j] = r[j] - alpha*Ap[j];
    }
